...
Run
bashpip install numpy pandas scipy yfinance
python main.py
...
pythones_rc = es_curr   # placeholder — no reduced set
...
Implement reduced set + proper MAR33.5(2)–33.6 ratio.
Per-ticker MLE GARCH fit + RFET.
IMA-DRC: multi-factor model, real ρ calibration, N_SIM ≥ 1e6.
Look-through for equity indices (MAR22.5).
Real data pipeline (loader, validation, hard-fail).
Test suite + regulatory benchmark reconciliation.
Model documentation (SR 11-7): data, assumptions, limits, validation.
